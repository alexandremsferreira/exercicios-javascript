...
// Correção: Você precisa prestar mais atenção no que estão pedindo, veja a palavra "método"
// brazilianFoods = ['Cozinha', null, 'Brigadeiro']
...
// Que porra é essa? Cara, porque você não trabalhou isso dentro uma template string
...
/** 
console.log(
    foodsInfo.replace('NUMERO_DE_ITENS', brazilianFoods.length)
        .replace('NOME_DO_ITEM_1', brazilianFoods[0])
            .replace('NOME_DO_ITEM_2', brazilianFoods[1])
                .replace('NOME_DO_ITEM_3', brazilianFoods[2])
)
*/

// Correção
// Veja a diferença! O código abaixo está mais simples e legível 
...
dogNames.pop()
// Passamos o dogNames[dogNames.length - 1] para dentro do console, afim de evitar
// mutabilidade
...
// Correção. Faltou fazer dos demais itens
...
// Que porra é essa?
// console.log(dogNames.toLocaleString().toLowerCase())
...
// Correção: Ale voce até resolveu o problema, mas veja que seu código
// poderia ter sido mais simples se usasse o slice e a template string
/**
const desset = (
        dogNames[0][3] + dogNames[0][4] + 
        dogNames[2][4] + dogNames[2][5] +
        dogNames[3][2] + dogNames[3][3]
    ).replace('n', 'd')
*/

// console.log(`A ${desset} é um doce à base de coco, tradicional na América Latina e em Angola.`)

// ['Chico', 'Zeca', 'Lilica', 'Nina']

// Ale veja a diferença. Usa a porra dos metodos mano! Eles te pouparam código
const desset = `${dogNames[0].slice(3)}${dogNames[1].slice(2)}${dogNames[3].slice(2)}`.replace('n', 'd')
